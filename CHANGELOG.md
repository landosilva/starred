...
- Favorites and History no longer repeat Unity's "Cross scene references are not supported" warning after every refresh when several scenes are open. Scene objects are now found without the Unity lookup that logged it again for each favorite.
...
- **Selection History window** (`Tools → Starred → History`). Filled automatically list of the last things you selected (both assets and scene GameObjects). Click the row to inspect, click the star to promote into Favorites.
...
- **Window option menus.** The three dot menu on each window mirrors the relevant preferences plus Clear / Open Preferences shortcuts.
